...
need_to_geocode_df = pd.read_parquet(
    '/storage1/fs1/jskrastins/Active/GeoCode/standardized_addr_eric.parquet', engine='pyarrow')[["stnd_type", "stnd_logr",
...
key_addresses_df = pd.read_parquet(
    '/storage1/fs1/jskrastins/Active/GeoCode/Brazil-Latest-points.parquet', engine='pyarrow')[["id", "housenumber", "street", "code_muni"]]
key_streets_df = pd.read_parquet(
    '/storage1/fs1/jskrastins/Active/GeoCode/Brazil-Latest-lines.parquet', engine='pyarrow')[["id", "street", "code_muni"]]
# merging the muni tables
muni_code_df = pd.read_parquet('/storage1/fs1/jskrastins/Active/GeoCode/muni_id.parquet', engine="pyarrow")[
...
def geocode_section(section_of_df, chunk_num):
    ...
    export_as_csv(
        df_output, f'/storage1/fs1/jskrastins/Active/GeoCode/output/addresses_matched_{load_from}_{load_to}_{chunk_num}.csv')
    export_as_csv(df_failed_addresses,
                  f'/storage1/fs1/jskrastins/Active/GeoCode/output/failed_addresses_{load_from}_{load_to}_{chunk_num}.csv')
...
def main():
    ...
    for idx, df in enumerate(need_to_geocode_df_parts):
        print(
            f"Processing from row {load_from} to {load_to}. Chunk {idx} out of {NUMBER_OF_CHUNKS}.")
